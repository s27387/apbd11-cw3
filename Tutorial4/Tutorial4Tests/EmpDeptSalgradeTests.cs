...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps
        .Join(grades, emp => true, grade => true, (emp, grade) => new { emp, grade })
        .Where(joined => joined.emp.Sal >= joined.grade.Losal && joined.emp.Sal <= joined.grade.Hisal)
        .Select(joined => new { joined.emp.EName, joined.grade.Grade })
        .ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo)
        .Select(grp => new { DeptNo = grp.Key, AvgSal = grp.Average(emp => emp.Sal) })
        .ToList();
    ...
    {
        var emps = Database.GetEmps();
        var deptAvg = emps.GroupBy(emp => emp.DeptNo)
        .Select(grp => new { DeptNo = grp.Key, AvgSal = grp.Average(emp => emp.Sal) });

        var result = emps
        .Join(deptAvg, emp => emp.DeptNo, avg => avg.DeptNo, (emp, avg) => new {emp.EName, emp.Sal, avg.AvgSal})
        .Where(x => x.Sal > x.AvgSal)
        .Select(x => x.EName)
        .ToList();
